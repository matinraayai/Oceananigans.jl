...
uᵢ(x, y, z) = - ϵ * f * ℓ * sech(y / ℓ)^2 * (z / Lz + 1) * (1 + 1e-1 * rand())
...
surface_bc_b = FluxBoundaryCondition(surface_buoyancy_flux, parameters = (Qᵇ = 1e-7, δ = grid.Ly / 20))
...
κ₄h = 1e-4 / day * grid.Δx^4 # [m⁴ s⁻¹] horizontal hyperviscosity and hyperdiffusivity
...
vertical_diffusivity = ConvectiveAdjustmentVerticalDiffusivity(convective_κz=1, background_κz=1e-5, convective_νz=1e-1, background_νz=1e-5)
...
wizard = TimeStepWizard(cfl=0.2, Δt=Δt, max_change=1.1, max_Δt=2Δt)
