...
for child_arch in Architectures
    ...
    arch = MultiArch(child_arch, ranks=(Rx, Ry, 1))
    grid = RectilinearGrid(arch, topology=topo, size=(Nx, Ny), extent=(1, 1))
    model = ShallowWaterModel(grid=grid, gravitational_acceleration=1.0)
    set!(model, h=1.0)

    @info "Warming up distributed shallow water model on $(typeof(child_arch)) on rank $local_rank..."

    time_step!(model, 1) # warmup

    @info "Benchmarking distributed shallow water model on $(typeof(child_arch)) on rank $local_rank..."
