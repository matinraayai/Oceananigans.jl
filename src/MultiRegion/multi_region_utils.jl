...
function assign_devices(p::AbstractPartition, ::CUDAGPU, dev::Number) 
    part     = length(p)
    repeat   = part ÷ dev
...
function assign_devices(p::AbstractPartition, ::ROCMGPU, dev::Number) 
    part     = length(p)
    repeat   = part ÷ dev
    leftover = mod(part, dev)
    devices  = []
    ...
    for i in 1:dev
        AMDGPU.default_device!(i-1)
        for j in 1:repeat
            push!(devices, AMDGPU.default_device())
        end
        if i <= leftover
            push!(devices, AMDGPU.default_device())
        end
...
function assign_devices(p::AbstractPartition, ::CUDAGPU, dev::Tuple) 
    part     = length(p)
...
function assign_devices(p::AbstractPartition, ::ROCMGPU, dev::Tuple) 
    part     = length(p)
    repeat   = part ÷ length(dev)
    leftover = mod(part, length(dev))
    devices  = []
...
# # Enable peer access by copying fake CuArrays between all devices
function maybe_enable_peer_access!(devices::NTuple{<:Any, <:AMDGPU.ROCDevice})

    fake_arrays = []
    for dev in devices
        switch_device!(dev)
        push!(fake_arrays, ROCArray(zeros(2, 2, 2)))
    end
